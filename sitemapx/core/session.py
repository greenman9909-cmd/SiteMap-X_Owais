...
def load_netscape_cookie_file(path: Path) -> list[dict]:
    ...
    for raw in path.read_text("utf-8", errors="ignore").splitlines():
        ...
        cookies.append({"domain": domain,"path": cookie_path or "/","secure": secure.upper() == "TRUE","expires": expires,"name": name,"value": value})
    return cookies
...
def _har_material(path: Path) -> tuple[dict[str, str], list[dict]]:
    ...
    for entry in entries:
        ...
        for cookie in req.get("cookies", []):
            ...
            if name:
                cookies.append({"name": name,"value": str(cookie.get("value", "")),"domain": str(cookie.get("domain", "")),"path": str(cookie.get("path", "/") or "/"),"secure": bool(cookie.get("secure", False))})
    return headers, cookies
...
def _set_cookie_both(cfg: Config, jar: aiohttp.CookieJar, http_client: httpx.AsyncClient | None, cookie: dict) -> None:
...
async def build_session(cfg: Config) -> SessionBundle:
    ...
    session = aiohttp.ClientSession(headers=headers, auth=basic_auth, cookie_jar=jar, timeout=timeout, connector=connector)
    http_client = None
    if cfg.http2:
        http_client = httpx.AsyncClient(headers=headers, auth=httpx_auth, http2=True, follow_redirects=True, timeout=cfg.timeout, proxy=cfg.proxy, max_redirects=cfg.redirect_limit)
    ...
    if cfg.login_url:
        ...
        if login_client is None:
            login_client = httpx.AsyncClient(headers=headers, auth=httpx_auth, follow_redirects=True, timeout=cfg.timeout, proxy=cfg.proxy, max_redirects=cfg.redirect_limit)
            temporary = True
        try:
            ...
            for cookie in login_client.cookies.jar:
                _set_cookie_both(cfg, jar, http_client, {"name": cookie.name,"value": cookie.value,"domain": cookie.domain,"path": cookie.path,"secure": cookie.secure})
        finally:
            if temporary:
...
def cookies_for_report(session: aiohttp.ClientSession, http2_client: httpx.AsyncClient | None = None) -> list[dict]:
    ...
    for morsel in session.cookie_jar:
        ...
        path = morsel["path"] or "/"
        merged[(name, domain, path)] = {"name": name,"value": morsel.value,"domain": domain,"path": path,"secure": bool(morsel["secure"]),"httponly": bool(morsel["httponly"])}
    if http2_client is not None:
        for cookie in http2_client.cookies.jar:
            merged[(cookie.name, cookie.domain or "", cookie.path or "/")] = {"name": cookie.name,"value": cookie.value,"domain": cookie.domain or "","path": cookie.path or "/","secure": bool(cookie.secure),"httponly": "httponly" in {str(k).lower() for k in cookie._rest}}
